...
class EvalCase(Base):
    ...
    gold_chunk_ids = Column(ARRAY(Integer), nullable=True)
    category = Column(String, nullable=False)

    def __repr__(self) -> str:
        return f"<EvalCase(id={self.id}, question={self.question[:30]}, category={self.category})>"
...
class EvalRun(Base):
    ...
    def __repr__(self) -> str:
        return f"<EvalRun(id={self.id}, git_sha={self.git_sha[:7]}, hybrid_recall={self.hybrid_recall})>"
