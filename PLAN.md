...
Live check of the corpus URLs — 7 of 8 fail:

| Scheme | HTTP |
| --- | --- |
| pm-kisan | 404 |
| mp-ladli-behna | 404 |
| ts-rythu-bandhu | no DNS |
| ap-ysr-cheyutha | no DNS |
| odisha-kalia | no DNS |
| ka-gruha-jyothi | 404 |
| pm-jjby | 404 |
| stand-up-india | 200 |

`data/raw/` does not exist locally, so nothing was ever cached from a successful fetch. Consequences:
...
| **0.5** | Re-source the corpus | Verify all 20 `source_url`s against live portals. Replace what's fixable, mark the rest `status: unverified` and exclude them. | pending |
